...
shuffled_glyph(int glyph)
{
    if glyph_is_normal_object (glyph) {
        ...
        if (!objects[obj].oc_name_known) return GLYPH_OBJ_OFF + gem_canon(objects[obj].oc_descr_idx);
...
NetHackRL::fill_obs(nle_obs *obs)
{
    ...
    if (obs->glyphs) { // eager identity mapping: every cell reflects the discoveries list now, not at its next
...
fill_message:
    ...
    if (obs->inv_state && !nle_partial) {
        ...
        for (otmp = invent; otmp && i < NLE_INVENTORY_SIZE;
            ...
            if (otmp->oclass == RING_CLASS && !(otmp->dknown && objects[otmp->otyp].oc_name_known)) st[1] = 0;
            ...
            st[5] = (signed char) (((otmp->owornmask
                                   ...
                                   | ((otmp->opoisoned && !Is_container(otmp)) ? 16 : 0)
                                   | (otmp->greased ? 32 : 0)
