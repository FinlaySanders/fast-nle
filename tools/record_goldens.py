    ...
    try:
        ...
        if actions_file:
            with open(actions_file) as af:
                actions = [int(t) for t in af.read().split()]
        ...
        with open(path, "w") as f:
            ...
            for i in range(max_steps):
                action = actions[i] if actions else pick_action(rng, obs[6], i)
...
def main():
    ...
    ap.add_argument("--actions-file", default=None,
                    help="record ONE episode driven by this whitespace-"
                         "separated keycode file instead of the scripted "
                         "policy (ground-truth any action stream via stock)")
