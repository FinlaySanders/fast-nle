...
coord *cc;
{
    ...
    levl[dig_x][dig_y].typ = ROOM, levl[dig_x][dig_y].rmflags = 0;
...
schar filltyp;
{
    if (trap && filltyp != ROOM && is_pit(trap->ttyp)) {
        ...
        t = *trap;
        levl[t.tx][t.ty].typ = filltyp, levl[t.tx][t.ty].rmflags = 0;
