...
boolean message;
{
    ...
    mnexto(mtmp);
    newsym(u.ux, u.uy);
    /* mtmp is used below and by callers (newcham): a level-tele trap here
       must defer its goto_level or the whole level -- mtmp included -- is
       freed under us (see level_tele) */
